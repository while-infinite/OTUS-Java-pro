...
public class DbServiceDemo {
    ...
    public static void main(String[] args) {
        ...
        //Get client add check ms before add to cache
        var beforeAddToCacheStart = System.nanoTime();
        var clientSecondSelected = dbServiceClient.getClient(clientSecond.getId())
                .orElseThrow(() -> new RuntimeException("Client not found, id:" + clientSecond.getId()));
        log.info("clientSecondSelected:{}", clientSecondSelected);
        log.info("client select before add to cache:{} ", (System.nanoTime() - beforeAddToCacheStart) / 1_000_000);

        //Get client add check ms after add to cache
        var afterAddToCacheStart = System.nanoTime();
        var checkCacheWork = dbServiceClient.getClient(clientSecond.getId())
                .orElseThrow(() -> new RuntimeException("Client not found, id:" + clientSecond.getId()));
        log.info("checkCacheWork:{}", checkCacheWork);
        log.info("client select after add to cache:{} ", (System.nanoTime() - afterAddToCacheStart) / 1_000_000);
